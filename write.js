...
function write(value) {
  if (!Array.isArray(value)) {
    if (typeof value == "function") return "[native " + value.name + "]";
    ...
    if (value.body) {
      return (value.name || "λ") + "(" + value.names.map(function (name) {
        return name.id;
      }).join(", ") + ")";
    }
    if (typeof value == "object" && "constant" in value) value = value.constant;
